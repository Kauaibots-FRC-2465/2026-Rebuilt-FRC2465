 ...
 * thread. Vision measurements can be added using {@link #addVisionMeasurement}.
 * Other odometry APIs such as {@link #resetPose} are also available. The
 * resulting pose estimate can be retrieved along with module states and
 * other information using {@link #getState}. Additionally, the odometry
 * thread synchronously provides all new state updates to a telemetry function
 * registered with {@link #registerTelemetry}.
 * <p>
 ...
 * the Back Left module, call {@code getModule(2);} to get the third
 * (0-indexed) module.
 */
...
> implements AutoCloseable {
    ...
    private SwerveDrivetrain(
        ...
        final var canbus = new CANBus(drivetrainConstants.CANBusName);
        @SuppressWarnings("unchecked")
        final var tmpModules = (SwerveModule<DriveMotorT, SteerMotorT, EncoderT>[])new SwerveModule<?, ?, ?>[modules.length];
        ...
        m_pigeon2 = new Pigeon2(drivetrainConstants.Pigeon2Id, canbus);
    ...
    public StatusCode configNeutralMode(NeutralModeValue neutralMode, double timeoutSeconds) {
        return StatusCode.valueOf(SwerveJNI.JNI_ConfigNeutralModeWithTimeout(m_drivetrainId, neutralMode.value, timeoutSeconds));
    }
     ...
     * perspective. This makes the current orientation of the robot the
     * X forward for field-centric maneuvers.
     * <p>
    ...
    /**
     * Resets the rotation of the robot pose to the given value from the
     * {@link SwerveRequest.ForwardPerspectiveValue#OperatorPerspective}
     * perspective. This makes the current orientation of the robot minus
     * {@code rotation} the X forward for field-centric maneuvers.
     * <p>
     * This is equivalent to calling {@link #resetRotation} with
     * {@code rotation.plus(getOperatorForwardDirection())}.
     *
     * @param rotation Rotation to make the current rotation
     */
    public void seedFieldCentric(Rotation2d rotation) {
        SwerveJNI.JNI_SeedFieldCentric(m_drivetrainId, rotation.getRadians());
    }
